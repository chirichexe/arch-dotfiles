...
# THIS SCRIPT CAN BE DELETED ONCE SUCCESSFULLY BOOTED!! And also, edit ~/.config/hypr/configs/SystemSettings.lua
...
if [ ! -f "$HOME/.config/hypr/.initial_startup_done" ]; then
	...
	if [ -f "$wallpaper" ]; then
		...
		awww query >/dev/null 2>&1 || { awww-daemon --format xrgb & sleep 0.5; }
